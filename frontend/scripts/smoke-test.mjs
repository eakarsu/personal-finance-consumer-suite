const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5850';
...
const adminCookie = await login('admin@personal-finance-consumer-suite.local', 'admin123');
const managerCookie = await login('manager@personal-finance-consumer-suite.local', 'manager123');
const analystCookie = await login('analyst@personal-finance-consumer-suite.local', 'analyst123');
